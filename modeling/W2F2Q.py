...
def WirelessWFQWorstCase(input_stream, count_channels, work_stream, queue_length, count_requests, discipline, weights_dict, **kwargs):
    currentTime = 0
    statGot = {'x': [0], 'y': [0]}
    statDone = {'x': [0], 'y': [0]}
    statRefused = {'x': [0], 'y': [0]}
    ...
    while (curGot < count_requests):
        if (timeDone == None or timeNew < timeDone):  # сначала поступает заявка
            ...
            GPS.add(
                {"name": 't' + str(curGot), "got": currentTime, "channel": numNew, "totalWork": random(work_stream),
            ...
            if (freeChannel != None):  # если канал свободен, обслуживаем заявку сразу
                ...
                channels[ch] = {"name": 't' + str(curGot), "got": currentTime, "start": currentTime,
                                "end": timeDone}
